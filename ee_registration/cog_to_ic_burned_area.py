...
# Input pattern: gs://smp-ee-files/30m_land_sent/{year}/{tile}.tif
...
def get_tif_list(project_name, bucket_name, prefix):
    """Lists all TIF files in a GCS bucket with a given prefix, excluding the backup folder."""
    ...
    tif_list = []
    for blob in blobs:
        # Skip original files in the backup directory
        if 'smp-ee-files' in blob.name:
            continue
...
def gcs_cogs_to_collection(cogs_df, collection_path):
    ...
    for cog_uri in cogs_df['tif']:
        # Parse path
        # Example: gs://smp-ee-files/30m_land_sent/2001/91.tif
        parts = cog_uri.split('/')
        filename = parts[-1]
        year_str = parts[-2]
